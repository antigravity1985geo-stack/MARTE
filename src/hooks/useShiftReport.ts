...
// ─── Saved Z-report from shift_summaries ───────────────────────
...
export function useShiftHistory(drawerId: string, limit = 30) {
  const [shifts,  setShifts]  = useState<ShiftSummaryRow[]>([])
    ...
    supabase
      ...
      .then(({ data }) => {
        const formatted = (data ?? []).map((s: any) => ({
          ...s,
          drawer: Array.isArray(s.drawer) ? s.drawer[0] : s.drawer
        }))
        setShifts(formatted as ShiftSummaryRow[])
        setLoading(false)
      })
  }, [drawerId, limit])
