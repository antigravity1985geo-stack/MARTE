// src/pages/clinic/WaitingRoomDisplay.tsx
...
import { useAllCountersDisplay } from '@/hooks/useQueue'
import { useQueueSound }         from '@/hooks/useQueue'
import { CounterDisplayState }   from '@/types/queue'
...
export default function WaitingRoomDisplay() {
  ...
  const { data: tenant, isLoading: tl } = usePortal(tenant_slug)
  
  const tenantId = tenant?.id ?? ''
  const clinicName = tenant?.name ?? 'კლინიკა'

  const { state, loading } = useAllCountersDisplay(tenantId)
          ...
          {tl || loading ? (
            <div style={{ gridColumn: `1 / -1`, textAlign: 'center', paddingTop: 80 }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
                <Loader2 style={{ width: 40, height: 40, color: 'rgba(255,255,255,0.2)' }} className="animate-spin" />
