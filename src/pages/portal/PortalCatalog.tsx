...
export const PortalCatalog = () => {
  ...
  const filteredItems = items?.filter(item => 
    ...
    (item.category || item.category_id || '').toLowerCase().includes(searchTerm.toLowerCase())
  );

  return (
    <div className="space-y-6 p-4 pb-24">
      <div className="flex flex-col gap-4">
        <h1 className="text-3xl font-black tracking-tighter">კატალოგი</h1>
                  ...
                  <CardContent className="p-0">
                    ...
                    <div className="p-5">
                      ...
                      <p className="text-xs text-muted-foreground line-clamp-2 mb-4">
                        {item.description || "დამატებითი აღწერა ხელმისაწვდომი არ არის."}
                      </p>
                      <Button asChild className="w-full h-12 rounded-xl portal-bg-primary text-white group">
                        <Link to={`/portal/${tenant_slug}/booking?serviceId=${item.id}`}>
                          დაჯავშნა <ArrowRight className="ml-2 h-4 w-4 group-hover:translate-x-1 transition-transform" />
                        </Link>
                      </Button>
                    </div>
                  </CardContent>
