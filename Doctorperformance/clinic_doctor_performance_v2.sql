-- ============================================================
-- DOCTOR PERFORMANCE DASHBOARD (ADAPTED FOR MARTE)
-- ============================================================

-- ─── 1. Dental chairs ────────────────────────────────────────
CREATE TABLE IF NOT EXISTS clinic_dental_chairs (
...
-- ─── 2. Appointment ← chair link ──────────────────────────────
ALTER TABLE clinic_appointments
  ADD COLUMN IF NOT EXISTS chair_id      UUID REFERENCES clinic_dental_chairs(id),
...
-- ─── 3. Transaction ← appointment link ────────────────────────
ALTER TABLE transactions
  ADD COLUMN IF NOT EXISTS appointment_id UUID REFERENCES clinic_appointments(id);

-- ─── 4. Core analytics view: doctor_daily_stats ──────────────
...
-- ─── 5. Core analytics view: doctor_revenue_stats ────────────
...
FROM transactions t
JOIN clinic_appointments a ON a.id = t.appointment_id
LEFT JOIN profiles p ON p.id = a.doctor_id
WHERE a.doctor_id IS NOT NULL
  AND t.status = 'completed'
GROUP BY 1, 2, 3, 4;

-- ─── 6. Chair utilization view ────────────────────────────────
...
FROM clinic_appointments a
JOIN clinic_dental_chairs dc ON dc.id = a.chair_id
WHERE a.chair_id IS NOT NULL
  AND a.status != 'cancelled'
GROUP BY 1, 2, 3, 4, 5;

-- ─── 7. Patient retention function ───────────────────────────
...
BEGIN
  ...
  GROUP BY c.doctor_id, p.full_name;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER STABLE;

-- ─── 8. Procedure breakdown per doctor ───────────────────────
...
BEGIN
  ...
  WHERE t.tenant_id = p_tenant_id
    ...
    AND t.status = 'completed'
    AND (p_doctor_id IS NULL OR a.doctor_id = p_doctor_id)
  GROUP BY a.doctor_id, p.full_name, ti.name
...
-- ─── Indexes ───────────────────────────────────────────────────
CREATE INDEX IF NOT EXISTS idx_clinic_appt_doctor_time ON clinic_appointments(doctor_id, start_time DESC);
CREATE INDEX IF NOT EXISTS idx_clinic_appt_chair_time  ON clinic_appointments(chair_id, start_time DESC);
CREATE INDEX IF NOT EXISTS idx_clinic_appt_patient     ON clinic_appointments(patient_id, start_time DESC);
CREATE INDEX IF NOT EXISTS idx_tx_appt_link            ON transactions(appointment_id);

-- ─── RLS on clinic_dental_chairs ──────────────────────────────
ALTER TABLE clinic_dental_chairs ENABLE ROW LEVEL SECURITY;
CREATE POLICY "tenant_chairs_v1" ON clinic_dental_chairs
  FOR ALL USING (auth_is_member(tenant_id));
