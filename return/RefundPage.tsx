...
function SearchStep({ onFound }: { onFound: () => void }) {
  ...
  // Expose result up via callback trick
  ;(SearchStep as any)._result = result
  ;(SearchStep as any)._clear  = clear
...
function SelectStep({
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Total */}
      {hasSelected && (
        <div className="bg-rose-900 text-white rounded-xl p-4 flex justify-between items-center">
...
export default function RefundPage() {
  ...
  const { createReturn, busy }              = useCreateReturn(tenantId)
